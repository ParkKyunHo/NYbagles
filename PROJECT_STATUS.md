...
## 📅 최종 업데이트: 2025년 7월 30일 (8차) - 상품 관리 RLS 및 UI 개선
...
### 19. 상품 관리 시스템 개선 ✅ (7/30 8차 업데이트)
- **RLS 정책 정리**
  - products 테이블의 중복된 RLS 정책 제거
  - product_categories 테이블의 중복된 정책 제거
  - 인증된 사용자를 위한 명확한 정책 유지
- **데이터 무결성 개선**
  - NULL category_id를 가진 상품들에 기본 '베이글' 카테고리 할당
  - store_products 테이블에 누락된 상품 자동 추가
- **UI/UX 개선**
  - 카테고리 관리 모달 내부 폰트 색상 개선 (연한 회색 → 진한 색상)
  - 테이블 헤더, 버튼, 텍스트 가독성 향상
- **배포 안정성**
  - ESLint 설정 간소화로 빌드 오류 해결
